...
public class PatternChecker {
    ...
    public static boolean isNum(String str) {
    	// +앞의 패턴으로만 구성된 문자열이 1개 이상 있어야 함 - nullcheck 가능
    	// (?는 ?앞의 패턴으로만 구성된 문자열이 없거나 1개)
    	return Pattern.matches("[0-9 ]+", str); 
    	// [0-9]는 한글자이므로 false. +는 한글자이거나 여러개이거나
    }
     
   //** 영문으로만 구성되었는지에 대한 형식 검사 *//*
    public static boolean isEng(String str) {
    	// 첫글자 A, 끝글자 z하면 대소문자 전체
   ...
   //** 이메일 형식인지에 대한 검사 --> "아이디@도메인"의 형식을 충족해야 한다. *//*
    public static boolean isEmail(String str) {
    	// +앞의 패턴이 1개 이상 있고, @는 1개이므로 수량기호x, 그 다음 패턴이 1개 이상 있어야 함  
    	return Pattern.matches("[A-z0-9]+@[A-z\\.]+", str);
    }
